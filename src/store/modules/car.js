...
export default {
  ...
  mutations: {
    ...
    deleteCartThenadd (state, { skuId, item }) {
      const index = state.list.findIndex(item => item.skuId === skuId)
  ...
  actions: {
    ...
    updateCart (ctx, goods) {
      ...
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) { // ctx.rootstate
          // 登录 TODO
        } else {
          // 本地
    ...
    checkAllCart (ctx, selectedStatus) {
      return new Promise((resolve, reject) => {
        if (ctx.rootState.user.profile.token) { // action的context 可以 结构出RootState，Store类似全局
          // 登录 TODO
        } else {
          ctx.getters.validList.forEach((e) => {
            ctx.commit('updateCart', { skuId: e.skuId, selected: selectedStatus })
            resolve()
          })
        }
      })
