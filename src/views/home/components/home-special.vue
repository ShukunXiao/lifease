<template>
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="container">
      <div class="special-item" v-for="item in list" :key="item.id">
        <RouterLink to="/">
          <img :src="item.cover" alt />
          <div class="meta">
            <p class="title ellipsis top">{{item.title}}
            <small class="sub ellipsis">{{item.summary}}</small></p>
...
import { findSpecial } from '@/api/getCategory'
...
export default {
  ...
  setup () {
    ...
    const result = lazyLoading(target, findSpecial)
    return { container: target, list: result }
  }
}
</script>
